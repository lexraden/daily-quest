...
export default function DailyTracker() {
   ...
   *
   * Only the count is held here — the list itself is fetched when the sheet
  ...
  useEffect(() => {
    ...
    // Load authenticated user and save name on first login
        const loadUser = async () => {
          try {
            // AuthContext has already established the session by the time this
            // page renders, so there is no separate authentication probe here.
  ...
  const handleOnboardingComplete = async (answers) => {
    try {
      // Prompt and schema live on the server; the response is already
      // sanitised and level-ordered there.
      const { quest_data } = await api.ai.generateQuests(answers, getLang());
      const sanitizedResult = sanitizeQuestData(quest_data, DEFAULT_QUEST_DATA);
      setQuestData(sanitizedResult);

      // One upsert replaces the old fetch-delete-create dance. The server owns
      // the trial clock, so it is not sent from here.
  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      try {
        ...
        const { data: cachedData, id: cachedId } = await getCachedUserData();
